...
if __name__ == '__main__':
    ...
    args.reduce_schedule = torch.optim.lr_scheduler.ReduceLROnPlateau(args.optimizer, mode='min', factor=0.5, patience=5,
    ...
    if args.apply_only == 0:
        ...
        for epoch in range(epoch_start, epoch_start + args.num_epochs):
            # Optimizer
            # lr = args.lr/pow(2,epoch-1) # Fix Me! Define your own learning rate
            # lr = args.lr/pow(10,(epoch-1)*2/(epoch_start + args.num_epochs-2))
            # optimizer = args.optimizer
            ...
            for data_slice_id in range(args.num_slices_test):
                ...
                pred_tmp = train_test.test_one_epoch(net, testloader, criterion, res, check_output = ifcheckOutput)
    ...
    for data_slice_id in range(args.num_slices_apply):
        ...
        pred_tmp = train_test.test_one_epoch(net, applyloader, criterion, applyRes)
        if len(pred) == 0:
            pred = pred_tmp
        else:
    ...
    if args.rank==0:
        ...
        # print(f"Detailed time: {applyRes['test_time']}\n Loss: {applyRes['test_loss']}\n Acc: {applyRes['test_acc']}")
